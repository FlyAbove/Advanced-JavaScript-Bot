...
module.exports = {
    ...
    async execute(message, args, client, bot){
        ...
        {message.channel.send({embed:{ title: `${red} Sorry but i dont have the courage to kick myself`, color: 'RED'}})}

       
        const member = message.mentions.members.first()
        if(!message.member.hasPermission('KICK_MEMBERS'))
        return message.channel.send({embed:{title: `${red} You don't have __KICK MEMBERS__ permission
        to use this command`, color: 'RED'}})
